...
def import_data_to_access():
    ...
    # データ型を文字列に変換
    df['指図'] = df['指図'].astype(str)

    # 指図フィールドの値から先頭の「000000」と「0000」を削除
    df['指図'] = df['指図'].str.replace(r'^000000', '', regex=True)
    df['指図'] = df['指図'].str.replace(r'^0000', '', regex=True)
    ...
    numeric_columns = ['発注残数', '正味単価', '購買発注量', '総重量', '安全在庫']
    ...
    # 日付フィールドの変換処理
    date_fields = ['登録日', '発注の入庫日付', '納入期日', '入庫日付', '再計画日付']
