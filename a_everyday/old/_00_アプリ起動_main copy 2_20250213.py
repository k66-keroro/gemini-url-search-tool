...
def main():
    ...
    for index, script in enumerate(scripts):
        ...
        try:
            ...
            if e.returncode == 3221225477:
                logging.info(f"[無視] スクリプトが異常終了しましたが無視: {script}")
            else:
    ...
    root.wait_window(progress_dialog.dialog)

    if failed_scripts:
        print("再実行するスクリプト:", failed_scripts)
        for script in failed_scripts:
            ...
            try:
                ...
                if e.returncode == 3221225477:
                    logging.info(f"[再実行無視] スクリプトが異常終了しましたが無視: {script}")
                else:
